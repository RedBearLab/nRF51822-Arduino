...
ble_error_t nRF51Gap::setAdvertisingData(const GapAdvertisingData &advData, const GapAdvertisingData &scanResponse)
{
    ...
    // ASSERT(ERROR_NONE ==
           // sd_ble_gap_adv_data_set(advData.getPayload(),    //_modify
                                   // advData.getPayloadLen(),
                                   // scanResponse.getPayload(),
                                   // scanResponse.getPayloadLen()),
           // BLE_ERROR_PARAM_OUT_OF_RANGE);
    nrf_err_check( sd_ble_gap_adv_data_set(advData.getPayload(),
                                   advData.getPayloadLen(),
                                   scanResponse.getPayload(),
                                   scanResponse.getPayloadLen()) );
    ...
    // ASSERT(ERROR_NONE == sd_ble_gap_appearance_set(advData.getAppearance()),  //_modify
           // BLE_ERROR_PARAM_OUT_OF_RANGE);
    nrf_err_check( sd_ble_gap_appearance_set(advData.getAppearance()) );
...
ble_error_t nRF51Gap::startAdvertising(const GapAdvertisingParams &params)
{
    ...
    // ASSERT(ERROR_NONE == sd_ble_gap_adv_start(&adv_para), //_modify
           // BLE_ERROR_PARAM_OUT_OF_RANGE);
    nrf_err_check(sd_ble_gap_adv_start(&adv_para));
...
{
    /* Stop Advertising */
    // ASSERT(ERROR_NONE == sd_ble_gap_adv_stop(), BLE_ERROR_PARAM_OUT_OF_RANGE); //_modify
    nrf_err_check( sd_ble_gap_adv_stop() );
...
ble_error_t nRF51Gap::disconnect(void)
{
    ...
    nrf_err_check(sd_ble_gap_disconnect(m_connectionHandle,
                                     BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION));           
...
ble_error_t nRF51Gap::setAddress(addr_type_t type, const uint8_t address[6])
{
    ...
    // ASSERT_INT(ERROR_NONE,  //_modify
               ...
               // BLE_ERROR_PARAM_OUT_OF_RANGE);
    nrf_err_check( sd_ble_gap_address_set(&dev_addr) );
